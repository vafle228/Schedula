...
class LessonService(ServiceBase):
    ...
    def create(self, data: Mapping[str, Any]) -> Lesson:
        ...
        The discipline defaults to the source topic's when omitted, and the id
        is allocated unless the caller supplies one.
        """
        topic_id = data.get("topic_id") or None
        topic = self._topics.get(topic_id) if topic_id else None
        discipline_id = data.get("discipline_id") or (
            topic.discipline_id if topic else None
        )
        lesson = Lesson(
            id=data.get("id") or ids.next_lesson_id(self._lessons),
    ...
    def patch(self, lesson_id: str, changes: Mapping[str, Any]) -> Lesson:
    ...
    def set_pin(self, lesson_id: str, pinned: bool) -> Lesson:
