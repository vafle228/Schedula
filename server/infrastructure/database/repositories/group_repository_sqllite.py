...
def _row_to_group(row: sqlite3.Row) -> Group:
    return Group(id=row["id"], major_id=row["major_id"], course=row["course"])


class GroupRepositorySqlLite(GroupRepository):
    """Stores groups, preserving insertion order via ``rowid``."""
    ...
    def add(self, group: Group) -> None:
        self._conn.execute(
            "INSERT INTO groups (id, major_id, course) VALUES (?, ?, ?)",
            (group.id, group.major_id, group.course),
        )
        self._conn.commit()

    def update(self, group: Group) -> None:
        self._conn.execute(
            "UPDATE groups SET major_id = ?, course = ? WHERE id = ?",
            (group.major_id, group.course, group.id),
        )
        self._conn.commit()
